...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        printf("Usage: ./inheritance [number of generations]");
        return 1;
    }

    int gen = (int)argv[1][0] - 48;
    // printf("%i\n", gen);
...
person *create_family(int generations)
{
    ...
    if (generations > 1)
    {
        ...
        child->alleles[0] = (child->parents[0])->alleles[random() % 2];
        child->alleles[1] = (child->parents[1])->alleles[random() % 2];
    }
